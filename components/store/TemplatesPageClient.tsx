...
export function TemplatesPageClient({ templates, isKenyan }: TemplatesPageClientProps) {
  ...
  return (
    <div className="min-h-screen bg-devcraft-bg">
      {/* Page Header — compact, no stat-pill row (was 3 colored badges competing with the heading) */}
      <section className="border-b border-devcraft-border">
        <div className="max-w-7xl mx-auto px-6 pt-20 pb-6">
          ...
          >
            {/* Breadcrumb */}
             <nav className="flex items-center gap-2 text-xs font-mono uppercase tracking-[0.12em] text-devcraft-slate mb-3">
              <span className="text-devcraft-slate-light">Home</span>
              <span className="text-devcraft-border">/</span>
              <span className="text-devcraft-foreground">Templates</span>
            </nav>
            ...
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              ...
              <div className="flex items-center gap-3">
               ...
               <h1 className="font-display text-3xl md:text-4xl text-devcraft-foreground tracking-tight">
              All Templates
            </h1>
            <p className="text-devcraft-slate-light mt-2 max-w-md text-sm">
              Production-ready codebases built with Next.js, React, and Tailwind CSS.{" "}
              <span className="text-devcraft-slate">
                {templates.length} template{templates.length !== 1 ? "s" : ""}, {freeCount} free.
              </span>
              {isKenyan && <span className="text-devcraft-emerald-glow ml-1">Prices in KES.</span>}
            </p>
          </motion.div>
        </div> 
      </section>

      {/* Toolbar: Search + Sort + View Toggle — uses the shared .glass utility (D6: filter bar is an approved use) */}
      <section className="glass sticky top-16 z-30 border-b border-devcraft-border">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                 className="w-full pl-10 pr-16 py-2.5 bg-devcraft-surface border border-devcraft-border rounded-lg
                           text-devcraft-foreground text-sm placeholder-devcraft-slate
              ...
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery("")}
                   className="absolute right-3 top-1/2 -translate-y-1/2 text-devcraft-slate hover:text-devcraft-foreground text-xs"
                 >
                  Clear
                </button>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={cn(
                  "sm:hidden flex items-center gap-2 px-3 py-2.5 rounded-lg border text-xs font-mono transition-colors",
                  showFilters
                    ? "bg-devcraft-violet/10 border-devcraft-violet/30 text-devcraft-violet-glow"
                    : "bg-devcraft-surface border-devcraft-border text-devcraft-slate-light hover:text-devcraft-foreground"
                )}
              >
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none pl-9 pr-8 py-2.5 bg-devcraft-surface border border-devcraft-border rounded-lg
                             text-devcraft-foreground text-xs font-mono cursor-pointer
                             focus:outline-none focus:border-devcraft-violet/50
                             transition-colors duration-200"
                >
              ...
              {/* View mode toggle */}
              <div className="flex items-center border border-devcraft-border rounded-lg overflow-hidden">
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === "grid"
                      ...
                      : "bg-devcraft-surface text-devcraft-slate hover:text-devcraft-foreground"
                  )}
                >
      ...
      <section className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-xs font-mono text-devcraft-slate">
            Showing{" "}
          <span className="text-devcraft-foreground font-medium">{filteredTemplates.length}</span> of{" "}
           <span className="text-devcraft-foreground font-medium">{templates.length}</span> templates
          </p>
    ...
    return (
      ...
      >
        <div className="w-16 h-16 rounded-2xl bg-devcraft-surface border border-devcraft-border flex items-center justify-center mb-4">
          <Search className="w-8 h-8 text-devcraft-slate" />
        </div>
        <h3 className="text-lg font-semibold text-devcraft-foreground mb-2">No templates found</h3>
        <p className="text-devcraft-slate text-sm max-w-sm">
      ...
      {templates.map((template, index) => (
        <motion.div
          ...
          className="flex items-center gap-4 p-4 bg-devcraft-card border border-devcraft-border rounded-xl
                     hover:border-devcraft-border-hover transition-colors duration-200 group"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <h3 className="text-sm font-medium text-devcraft-foreground group-hover:text-devcraft-violet-glow transition-colors truncate">
              {template.title}
            </h3>
            <p className="text-xs text-devcraft-slate truncate">{template.description || ""}</p>
          </div>
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-sm font-mono text-devcraft-foreground">
              {template.isFree ? "Free" : `$${template.priceUsd}`}
            </span>
            <a
              href={`/template/${template.slug}`}
              className="px-4 py-2 rounded-lg bg-devcraft-violet text-devcraft-bg text-xs font-medium
